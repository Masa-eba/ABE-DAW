...
24. `Command + Option + U`で選択中MIDIトラックへドラムパターンを生成
25. `Command + Option + Shift + U`で選択中MIDIトラックへドラムフィルを生成
26. `Command + Option + Y`で選択中MIDIトラックへリードメロディを生成
27. MidiClipを掴んでタイムライン上または別MIDIトラックへ移動
28. MidiClipの右端をドラッグして長さを変更
29. `Duplicate Clip` / `Delete Clip`でMidiClipを編集
30. `Quantize`で選択中MidiClipを現在のスナップグリッドへ揃える
31. `Command + ↑ / ↓`で選択中MidiClipをオクターブ上下に移調
32. `Command + ] / [`で選択中MidiClipのVelocityを上下
33. `Command + Option + Shift + [ / ]`で選択中MidiClipをDouble-time / Half-timeに変換
34. `Zoom`でタイムラインの横方向表示倍率を調整
35. `Command + ← / →`で選択中Clipを現在のスナップグリッド単位で左右へ移動
36. `Command + J`で選択中Clipを再生位置へ移動
37. `Option + ← / →`で再生位置を前後の拍へ移動
38. `Option + Shift + ← / →`で再生位置を前後の小節へ移動
39. `Option + M`で現在位置にマーカーを追加
40. `Command + Option + M`で選択中Clipの先頭にマーカーを追加
41. `Option + Shift + M`で再生位置付近のマーカー名を変更
42. `Option + Delete`で再生位置付近のマーカーを削除
43. `Option + , / .`で前後のマーカーへ移動
44. `M / S / Volume / Pan`でトラックを調整
45. `Loop`でプロジェクト長の範囲を繰り返し再生
46. `Panic`または`Esc`で鳴りっぱなしのMIDIノートを停止
47. `Export Mix`でミックス結果を書き出す
48. `Export Track`で選択トラックだけをStemとして書き出す
...
- VST / AUプラグインには未対応
- MIDI CC編集は未実装
- AI Chords / AI Bass / AI Arp / AI Drums / AI Drum Fill / AI Melodyはローカルの簡易MIDI生成で、外部AI APIは未接続
