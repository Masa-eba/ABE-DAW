...
5. `Dup Track`で選択中トラックを複製
6. MIDIトラックを選択して`R`を有効化
7. 画面鍵盤または外部MIDIキーボードで発音
8. `Record`で録音
9. AudioClipを掴んでタイムライン上で移動
10. 再生位置線をAudioClip内に置いて`Split`で分割
11. `Duplicate Clip` / `Delete Clip`でAudioClipを編集
12. `Fade In` / `Fade Out`で1秒フェードを適用
13. `Command + = / -`で選択中AudioClipのClip Gainを上下
14. `AI Chords`で選択中MIDIトラックへコード進行を生成
15. MidiClipを掴んでタイムライン上または別MIDIトラックへ移動
16. `Duplicate Clip` / `Delete Clip`でMidiClipを編集
17. `Quantize`で選択中MidiClipを16分グリッドへ揃える
18. `Command + ↑ / ↓`で選択中MidiClipをオクターブ上下に移調
19. `Command + ] / [`で選択中MidiClipのVelocityを上下
20. `Zoom`でタイムラインの横方向表示倍率を調整
21. `Command + ← / →`で選択中Clipを16分単位で左右へ移動
22. `M / S / Volume / Pan`でトラックを調整
23. `Loop Clip`で選択中AudioClip / MidiClipの範囲を繰り返し再生
24. `Loop`でプロジェクト長の範囲を繰り返し再生
25. `Export Mix`でミックス結果を書き出す
