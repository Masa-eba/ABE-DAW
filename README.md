...
8. `Duplicate Clip` / `Delete Clip`でAudioClipを編集
9. `AI Chords`で選択中MIDIトラックへコード進行を生成
10. `M / S / Volume`でトラックを調整
11. `Loop`でプロジェクト長の範囲を繰り返し再生
12. `Export Mix`でミックス結果を書き出す
...
- `TimelineComponent`
  ...
  - AudioClipのドラッグ移動、ファイルドロップ、スナップを担当

## 現在の制限

- ピアノロール編集は未実装
- AudioClipのカット、分割、フェードは未実装
- Audioタイムストレッチは未実装
- VST / AUプラグインには未対応
